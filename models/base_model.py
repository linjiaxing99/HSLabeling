...
def build_base_model(opt):
    ...
    elif opt.base_model == 'Deeplab1':
        model = DeepLab1(backbone=opt.backbone, in_channels=opt.in_channels)

    elif opt.base_model == 'Deeplab2':
        model = DeepLab2(backbone=opt.backbone, in_channels=opt.in_channels)

    elif opt.base_model == 'Deeplab3':
        model = DeepLab3(backbone=opt.backbone, in_channels=opt.in_channels)

    elif opt.base_model == 'Deeplab14':
        model = DeepLab14(backbone=opt.backbone, in_channels=opt.in_channels)
