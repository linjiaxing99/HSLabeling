...
class DeepLab1(nn.Module):
    def __init__(self, backbone, in_channels, pretrained=True,
        ...
        if 'resnet18' in backbone:
            ...
            self.backbone = mit_b0()
            checkpoint = torch.load('/media/hlf/Luffy/WLS/ContestCD/pretrained_former/segformer_b0.pth',
            ...
            self.backbone = mit_b3()
            checkpoint = torch.load('/media/hlf/Luffy/WLS/ContestCD/pretrained_former/segformer_b3.pth',
                                    map_location=torch.device('cpu'))
            self.backbone.load_state_dict(checkpoint['state_dict'])
            print('load pretrained transformer')
            low_level_channels = 64
    ...
    def forward(self, x):
        ...
        x = self.decoder(feature, x[0])
        # x = F.interpolate(x, size=(h, w), mode='bilinear')
        feat = F.interpolate(feat, size=(h_, w_), mode='bilinear')
...
class DeepLab14(nn.Module):
    def __init__(self, backbone, in_channels, pretrained=True,
                 output_stride=16, aspp_stride=8, freeze_bn=False, **_):

        super(DeepLab14, self).__init__()
        assert ('resnet' or 'resnest' in backbone)
    ...
    def forward(self, x):
        ...
        x = self.decoder(feature, x[0])
        # x = F.interpolate(x, size=(h, w), mode='bilinear')
        feat1 = F.interpolate(feat1, size=(h_, w_), mode='bilinear')
        feat = torch.cat([feat1, feat4], dim=1)
