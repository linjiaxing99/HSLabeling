...
def train_validate(opt, train_txt_name, current_budget, surface_ratio=1, line_ratio=1, point_ratio=1):
    device = torch.device("cuda:0")
    ...
    train_txt_path = os.path.join(data_inform_path, train_txt_name)
    ...
    for epoch in range(opt.num_epochs):
        ...
        train(opt, epoch, model, train_loader, optimizer, surface_ratio, line_ratio, point_ratio, logger, device)
        ...
        if metric < best_metric:
            ...
            torch.save({'state_dict': model.state_dict()},
                       os.path.join(checkpoint_path, 'model_best_' + str(int(current_budget)) + '.pth'))
            best_metric = metric
...
def train(opt, epoch, model, loader, optimizer, surface_ratio, line_ratio, point_ratio, logger, device):
    model.train()
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        loss = surface_ratio * surface_seg_loss + line_ratio * line_seg_loss + point_ratio * point_seg_loss
        # loss = point_seg_loss
        ...
        if last_batch or batch_idx % log_interval == 0:
            logger.info('Train:{} [{:>4d}/{} ({:>3.0f}%)] '
                    ...
                    'surface_ratio:({surface_ratio:>6.4f}) '
                    'line_ratio:({line_ratio:>6.4f}) '
                    'point_ratio:({point_ratio:>6.4f}) '
                    'LR:{lr:.3e''} '.format(
                        epoch,
