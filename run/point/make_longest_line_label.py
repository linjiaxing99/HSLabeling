...
def find_longest_segment(mask):
    ...
    approxs_record = 0
    while True:
        approxs.clear()  # 清空 approxs 以便重新计算
        for contour in contours:
            epsilon = epsilon_factor * cv2.arcLength(contour, True)  # 更新 epsilon 值
            approx = cv2.approxPolyDP(contour, epsilon, True)
            approxs.extend(approx)
        if len(approxs) == approxs_long:
            approxs_record = approxs_record + 1
        else:
            approxs_long = len(approxs)
        if approxs_record > 20:
            break
        # 如果 approxs 长度小于 200，跳出循环
        if len(approxs) < 50:
            break
        # 如果长度大于 200，增加 epsilon_factor
        epsilon_factor += 0.02
