...
from DataProcess.TransPostLabel import value_to_rgb
import random
import ttach as tta
...
def main():
    ...
    model = Seg_Net(opt)
    check_path = '/home/isalab206/LJX/HSLabeling-master/al_seg_train_20000_HSLabeling_loveda/save/potsdam/Point'
    checkpoint = torch.load(check_path + '/checkpoint' + '/model_best.pth', map_location=torch.device('cpu'))
    ...
    for batch_idx, batch in enumerate(tqdm(loader)):
        ...
        crf_out = output.argmax(dim=1).squeeze(0).cpu().numpy()
        # visual = value_to_rgb(crf_out[:, :, np.newaxis])
        # imsave(check_path + '/predict_test' + '/' + filename[0], visual)
        # save_pred_anno_numpy(crf_out, predict_path, filename, dict=potsdam_color_dict, flag=True)
